...
extern uint8_t uart1_tx_buffer[50];
...
{
  if((UART1->SR & 0x80) && (UART1->CR2 & 0x80)){
      /* clear the TXE flag */
      UART1->SR &= (uint8_t)~(UART1_SR_TXE);
      
      UART1_SendData8(*uart1.tx_buffer_ptr++);
      if(--uart1.size==0U){
        /* disable TXE interrupt */
        UART1->CR2 &= (uint8_t)(~(1 << 7));
        /* enable TC interrupt */
        UART1->CR2 |= (uint8_t)(1 << 6);
      }
  }else if((UART1->SR & 0x40) && (UART1->CR2 & 0x40)){
    /* clear the TC flag */
    ...
    uart1.tx_state = UART_STATE_READY;
  }
}
 ...
 INTERRUPT_HANDLER(UART1_RX_IRQHandler, 18)
{
    ...
    {
        UART1->SR = (uint8_t)~(UART1_SR_OR);
    }
    
    /* Read one byte from the receive data register */
    queue_in(&queue,(uint8_t)UART1->DR);
    
//    if ((UART1->SR & (uint8_t)UART1_SR_RXNE) != (uint8_t)0x00)
//    {
//        /* Read one byte from the receive data register */
//        queue_in(&queue,(uint8_t)UART1->DR);
//    }
